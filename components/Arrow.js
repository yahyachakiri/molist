...
export const Arrow = ({left, right, bg, disabled, onClickAction}) => {
    return (
        <button onClick={onClickAction} className={`w-[70px] h-[70px] flex items-center justify-center ${bg ? bg : 'bg-darkBg'} ${disabled ? 'opacity-60 cursor-auto' : 'transition duration-300 hover:bg-main'}`}>
            {
                right ?
